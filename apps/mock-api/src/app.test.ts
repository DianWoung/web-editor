...
test('GET /api/assets/models/:assetId also serves a model from import packs when data/equipment is missing it', async () => {
  const dataRoot = await setupFixture()
  await mkdir(path.join(dataRoot, 'assets', 'import-packs', 'demo-pack', 'chw_pump_v1'), { recursive: true })
  await writeFile(path.join(dataRoot, 'assets', 'import-packs', 'demo-pack', 'chw_pump_v1', 'model.glb'), 'import-pack-glb')
  await writeFile(
    path.join(dataRoot, 'equipment', 'chw_pump_v1', 'asset.json'),
    JSON.stringify(
      {
  ...
  assert.equal(modelRes.status, 200)
  assert.equal(modelRes.text, 'import-pack-glb')
})
