...
- `/` → 重定向到 **`/scenes`**
- **`/assets`**：资产配置中心（新建、编辑、连接点配置、绑定占位、模型上传、发布/下线/删除）；当前定位为场景工作区的次级配置入口
- **`/scenes`**：场景运营工作台（创建、切换、运行态摘要、进入总览/预览/编辑、删除）
...
- 顶部导航只保留 `场景` 作为一级主心智
- `/scenes` 使用“浅色信息层 + 深色实时态舞台”的 SaaS 工作台结构
- `编辑 / 预览 / 总览` 不再是三个独立产品页，而是场景工作区下的三个子工作区
...
- `GET /api/scene/library` / `GET|PUT|DELETE /api/scene/library/:sceneId`：命名场景列表、内容与删除
...
- **场景工作台流转**：先在 `/scenes` 用弹框填写 `场景名称 + 场景备注` 创建场景，再从列表或右侧摘要进入 `总览 / 预览 / 编辑`；删除只在工作台内进行，且带二次确认。
- **编辑页收敛**：编排页顶部只保留 `场景名称`、`场景备注`、`保存`、`撤销`、`返回场景工作台`；底部 JSON/调试面板已经移除出主流程。
- **总览页定位**：命名场景总览承接场景级运行态摘要与深色实时态主视区，保留设备详情跳转能力，为后续实时数据接入预留位置。
- **预览页定位**：预览页是只读效果页，只保留场景摘要、流动状态开关和进入编辑入口。
- **压测**：仍可通过 URL 查询参数 **`?stress=80`**（上限 500）感受帧率并记录卡顿（阶段 1 性能基线）。
...
- 命名场景总览从 `/scenes/:sceneId/overview` 进入，先加载指定场景，再轮询 runtime overview。
...
- `npm test -w twin-web` 当前会同时跑 Node 侧 API/store 测试和 Vitest 页面级测试，覆盖 runtime 接线、场景工作台、预览页、资产管理页与编排页命名场景加载。
...
- 无撤销/重做、无多人协作。
- 顶层导航不再把资产管理作为与场景并列的一级主入口。
- 预览页是只读场景效果页，仅保留场景摘要、流动状态开关和进入编辑入口。
